...
Meldra adds the `meldra-profile` status key. Widget keys include `plan-todos`, `rpc-demo`, `widget-above`, `widget-below`, and the local project Extension's `prompt-url`.
