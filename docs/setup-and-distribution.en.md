...
| Dual Windows x64 installers | `SUPPORTED` | [`v0.1.0-preview.4`](https://github.com/Slapq/MetaPi/releases/tag/v0.1.0-preview.4) provides bundled-Node and system-Node variants |
...
MetaPi provides a Windows installer and retains a planned npm Bootstrap as the two first-install entries. Both use the same Setup service instead of maintaining separate installation semantics.

### Windows installer

The Release provides two installers for Windows 10 build 19041+ / Windows 11 x64:

- `MetaPi-Setup.exe` targets machines without a preinstalled Node.js and bundles official Node.js 24.19.0;
- `MetaPi-Setup-NodeJS.exe` uses the machine's existing Node.js. Missing Node/npm or a version below 22.19.0 produces truthful guidance but does not block installation.

Both variants bundle the official Windows Terminal 1.24.11911.0 x64 unpackaged distribution in `.portable` mode. The installer adds `metapi` to the current-user PATH, so the CLI works in any newly opened PowerShell, cmd, Git Bash, Windows Terminal, or VS Code terminal. The desktop `MetaPi` shortcut defaults to the bundled Windows Terminal without binding MetaPi to it or changing the system default terminal.

Both variants share one `AppId` and can upgrade each other in place. The default location is `%LOCALAPPDATA%\Programs\MetaPi`, with no administrator requirement. Upgrades preserve `~/.metapi`; uninstall removes the application, shortcut, and MetaPi's own PATH entry while preserving Profiles, credentials, models, and Sessions.
...
The source distribution now provides the shared Setup service. A clean first MetaPi user initialization provisions the Starter Profile Bundle; existing users can run:
...
1. initializes the MetaPi user directory;
2. installs or restores the project-maintained MetaPi Starter Profile Bundle;
