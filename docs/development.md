...
需要 Node.js `>= 22.19.0`、npm 和 Git。Windows 还需要 Git Bash；修改原生 TUI 时需要 Visual Studio C++ Build Tools 与 Windows SDK。
...
DSH 固定为 `0.1.0-rc.6`。开发时以当前安装包类型、Harness 源码、ApiProxy、事件和真实 runtime probe 为依据。
...
长期产品决定和公开领域术语写入 ADR；复杂证据链放入 `docs/investigations/`。调查文档不能替代 ADR。
