...
There is no authoritative MetaPi npm release source in the current repository. Do not substitute the official Pi package
for MetaPi. `metapi update --self` is disabled. The standalone Windows installer, scoped npm Bootstrap, Starter Bundle Setup, Provider/model/Scout onboarding, and desktop shortcut are `PLANNED`, not currently available. See the confirmed [Setup and Distribution Contract](setup-and-distribution.en.md).
...
`default` is positioned as a minimal MetaPi Starter Profile continuously tuned by the project team, using a small set of defaults for a stable, direct daily development entry. A clean source launch receives MetaPi's Profile, WorkSpace, and `/config` built-in surfaces. The project-maintained Provider manager, Scout, and workflow Starter Bundle remain part of the planned Setup delivery and are not yet clean-install defaults.
...
Use `/workspace` to show the binding. `metapi-workspace` is a built-in package for every ordinary MetaPi Profile. The planned Windows Setup will create one current-user desktop shortcut that launches `metapi --profile default --workspace`; that installation path is not yet released.
