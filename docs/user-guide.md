...
Windows x64 用户可以从 [`v0.1.0-preview.4`](https://github.com/Slapq/MetaPi/releases/tag/v0.1.0-preview.4) 下载双安装器：`MetaPi-Setup.exe` 内置 Node.js，`MetaPi-Setup-NodeJS.exe` 使用系统 Node.js。两版都内置 portable Windows Terminal，桌面快捷方式默认使用它；安装器同时把 `metapi` 加入当前用户 PATH，因此任意新开的 PowerShell、cmd、Git Bash、Windows Terminal 或 VS Code terminal 都能运行 MetaPi。安装器当前未签名，Windows 可能显示 Unknown publisher 或 SmartScreen 提示。
...
安装器支持 Windows 10 build 19041+ / Windows 11 x64。Bash tool 仍需要 Bash，推荐 Git for Windows。使用源码入口时还需要 Node.js `>=22.19.0`、npm 与 Git。
...
向导始终按 Provider → 模型 → Scout 顺序展示三个步骤，不会跳过已有配置。每一步标注“已配置 / 部分配置 / 未配置”；已配置步骤可保留并继续或重新配置，未完成步骤连续打开和等待对应的 `/login`、`/model`、`/scout` 原生界面。取消或失败时留在当前步骤，可重试、仅跳过本轮或退出；最后可继续打开 `/config`。
