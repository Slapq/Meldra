...
| ⚙️ | **统一插件配置** | 普通 Profile 自动获得 `/config` TUI，Provider 与 Scout onboarding 已进入 Setup 计划 |
...
| 安装入口 | 状态 | 说明 |
|---|---|---|
| 从源码运行 | `SUPPORTED` | 当前唯一可靠入口 |
| `MetaPi-Setup.exe` | `PLANNED` | 计划中的 Windows 双击安装器，尚无下载文件 |
| scoped npm Bootstrap | `PLANNED` | MetaPi organization 与包名尚未确认，不提供假命令 |
| Starter Bundle + 配置向导 | `PLANNED` | 将引导 Provider、模型、Scout 与 thinking level，并允许跳过 |
| Windows 桌面快捷方式 | `PLANNED` | 安装后计划启动 `metapi --profile default --workspace` |
...
- Node.js `>= 22.19.0`
- npm 与 Git
- Windows 需要 Bash；推荐 Git for Windows
