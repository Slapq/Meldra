...
const EN: I18n = {
	...
	baseUrlHint: "Enter the API root (for example https://host/v1). Discovery also tries origin/v1/models automatically.",
	fieldApiKey: "API Key:",
...
function metadataFromRaw(raw: any): DiscoveredModel | null {
	...
	const capabilities = Array.isArray(raw.capabilities) ? raw.capabilities.map((x: any) => String(x).toLowerCase()) : [];
	...
	return {
		...
		rich: Boolean(
			raw.displayName ||
				...
				supported.length ||
				capabilities.length ||
				positiveNumber(raw.contextWindow, raw.context_window, raw.context_length, raw.top_provider?.context_length) ||
...
function inpKey(i: Inp, data: string): boolean {
	...
	if (data.startsWith("\x1b[200~")) {
		const content = data.replaceAll("\x1b[200~", "").replaceAll("\x1b[201~", "");
		if (content.length > 0) {
			let insert = "";
				...
				try {
					pi.registerProvider(result.name, {
						...
						models: result.config.models.map((m) => ({
							...
							reasoning: m.reasoning,
							...(Object.keys(m.thinkingLevelMap).length > 0 ? { thinkingLevelMap: m.thinkingLevelMap as any } : {}),
...
class FormComponent implements Focusable {
	...
	private handleProvAdvanced(data: string): void {
		...
		switch (f) {
			...
			case "pThinkingFormat":
				if (matchesKey(data, Key.left))
					this.pThinkFmtIdx = (this.pThinkFmtIdx - 1 + COMPAT_THINKING_FORMATS.length) % COMPAT_THINKING_FORMATS.length;
				else if (matchesKey(data, Key.right) || matchesKey(data, Key.enter))
	...
	private addPiAgentHeaders(): void {
		...
		for (const [key, value] of Object.entries(preset)) {
			if (!Object.keys(headers).some((existing) => existing.toLowerCase() === key.toLowerCase())) headers[key] = value;
		}
	...
	private renderOverview(w: number): string[] {
		...
		this.row(lines, w, t.provAdvanced, "provAdvanced", cf, th.fg("muted", `${t.provCompatHint} · ${compatCount}`), 26);
		lines.push("");
