...
`dsh.profile.bundles` roster is the published `@deepseek-ai/dsh-base` plus `@deepseek-ai/dsh-web-app` rc.6 layers,
...
After the user confirms one model, `AgentSession` calls the DSH Runtime's generic model-selection hook before committing
the Pi model change. DSH writes only that provider/model route into the live `llm-pi-ai` Settings namespace, stores a
resolved key through the Harness credential service under a stable reference rather than in Settings, and then calls
native `session.selectModel`. Pi persists its model-change entry and Profile preference only after all three operations
succeed. Opening or cancelling `/model` performs no DSH write, and MetaPi does not bulk-copy its model catalog or auth
files. The bundled rc.6 `llm-pi-ai` configuration supports `openai-completions`, `openai-responses`, and
...
Other MetaPi APIs remain visible in Pi's catalog but are rejected before any Harness Settings write because rc.6 cannot
...
It is read-only because rc.6 exposes no `jobs.*` unary control or output API; background output remains available
...
command slot of `session.prompt`; it does not reproduce plan policy. The Todo menu is read-only because rc.6 exposes
...
The source Session remains unchanged; a running turn is cancelled and must report idle before the fork, the child
becomes active, and text plus transient image bytes are restored atomically through Pi's editor draft. First-turn
messages and content shapes outside the shared Pi/Harness text-plus-images contract remain explicitly unavailable.
...
- dedicated Session ZIP export/download surface (rc.6 exposes this as an HTTP streaming route, not a stdio binary RPC);
- trajectory view, projection details, context breakdown, deliverables, and message feedback.

DSH remains pinned to `0.1.0-rc.6`; its release-candidate protocol and event shapes may change.
