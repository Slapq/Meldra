...
Execute another slash command through the active host and await that command's existing completion semantics:

```typescript
pi.registerCommand("onboard", {
  handler: async (_args, ctx) => {
    if (!(await ctx.executeCommand("/login"))) return;
    await ctx.executeCommand("/model");
  },
});
```

The calling Extension owns orchestration only. InteractiveMode owns built-in command dispatch, while each target built-in or Extension command continues to own its UI, state writes, cancellation, errors, and side effects. The command context and dispatcher are session-bound and require no separate teardown.

Behavior:
- only a known slash command is dispatched; unknown or multiline input returns `false` and is never sent to the model;
- Extension commands execute through their registered handler and are awaited;
- built-in commands reuse the same InteractiveMode path as editor submission;
- `true` means the command was available and dispatched, not that the user completed its optional UI or produced a successful business result;
- cancellation and errors retain the target command's existing behavior, so an orchestrator should inspect authoritative state before advancing;
- print, JSON, and RPC command contexts currently return `false`; this host capability is supported by the interactive TUI.
...
pi.registerCommand("use-review-profile", {
  handler: async (_args, ctx) => {
    const result = await ctx.switchProfile?.("review");
...
Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `pi.appendEntry()` and do not participate in LLM context. Active-Session HTML export (`/export` and `/share`) invokes the same renderer with `expanded: true`, converts its themed terminal output to HTML, and strips the private `customType`/`data` payload from the embedded HTML Session data. A successfully rendered custom transcript may be exported before Pi's deferred Session JSONL file is created; ordinary unflushed Pi Sessions without rendered custom content keep the existing “Nothing to export yet” error. Entries without a registered renderer remain invisible and do not expose their private payload. If a registered renderer throws, export fails explicitly instead of producing an incomplete file. Standalone file export has no active ExtensionRunner, so it does not render custom entries.
...
// Text input (unchanged; displayed as entered)
const name = await ctx.ui.input("Name:", "placeholder");

// Write-only secret input (TUI only; rendered with a mask)
const token = await ctx.ui.secretInput("API token:", "secret value");
...
`secretInput()` is additive: existing `input()` calls, rendering, return values, and plugin behavior are unchanged. In TUI mode the secret is masked while editing, but the returned value is the exact submitted string. The value is not logged or persisted by the dialog. RPC, print, and other no-UI modes return `undefined` instead of requesting an unmasked value; extensions must handle that result explicitly.
...
- `input()` and `secretInput()` return `undefined`
...
// Atomically replace/read a text + image draft (TUI returns true)
const accepted = ctx.ui.setEditorDraft({
  text: "Inspect this image",
  images: [{ type: "image", data: base64, mimeType: "image/png" }],
});
const draft = ctx.ui.getEditorDraft();
...
`setEditorDraft()` replaces the TUI editor text and its transient image list atomically. Draft images are submitted once through the ordinary `AgentSession.prompt({ images })` path and then cleared; clearing the editor clears both text and images. `setEditorText()` changes only text and preserves any draft images. RPC accepts text-only drafts but returns `false` without changing the editor when images are present; `getEditorDraft()` returns `undefined` where the host cannot synchronously expose complete draft state. Image bytes are never appended to the Pi Session merely by setting a draft.
...
| `provider-manager-package/` | Compact searchable Provider/model management package with explicit cancellable discovery | `registerProvider`, `unregisterProvider`, `ui.custom`, `SelectList` |
