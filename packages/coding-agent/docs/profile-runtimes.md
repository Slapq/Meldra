...
When a Runtime implements `selectModel(model)`, Pi's native `/model` selector, direct `/model provider/id` command, Ctrl+P model cycling, scoped-model cycling, and model-selection validation remain authoritative. The Runtime receives only the model the user confirmed or cycled to. Pi waits for the Runtime before changing agent state, appending a model-change entry, or persisting the Profile preference; rejection leaves the previous Pi selection intact. A Runtime that omits the hook retains ordinary Pi behavior.
