...
Options:
  -l, --local       Edit Current WorkSpace overrides (${CONFIG_DIR_NAME}/settings.json)
  -a, --approve     Trust WorkSpace-local files for this command with -l
  -na, --no-approve Ignore WorkSpace-local files for this command with -l
`);
}
...
Options:
  -l, --local       Install for the Current WorkSpace (${CONFIG_DIR_NAME}/settings.json)
  -a, --approve     Trust WorkSpace-local files for this command
  -na, --no-approve Ignore WorkSpace-local files for this command
...
Options:
  -l, --local       Remove from Current WorkSpace settings (${CONFIG_DIR_NAME}/settings.json)
  -a, --approve     Trust WorkSpace-local files for this command
  -na, --no-approve Ignore WorkSpace-local files for this command
...
${SELF_UPDATE_ENABLED ? `Update ${APP_NAME}, installed packages, or model catalogs.` : `Update installed packages or model catalogs. ${APP_NAME} distribution self-update is disabled.`}

Options:
  --self                  ${SELF_UPDATE_ENABLED ? `Update ${APP_NAME} only (default when no target is given)` : "Unavailable in this build"}
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   ${SELF_UPDATE_ENABLED ? `Update ${APP_NAME} and installed packages` : "Update installed packages; distribution remains unchanged"}
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 ${SELF_UPDATE_ENABLED ? `Reinstall ${APP_NAME} even if the current version is latest` : "No effect while distribution self-update is disabled"}

Short forms:
  ${APP_NAME} update                ${SELF_UPDATE_ENABLED ? `Update ${APP_NAME} only` : "Report that distribution updates are disabled"}
  ${APP_NAME} update --all          ${SELF_UPDATE_ENABLED ? `Update ${APP_NAME} and all extensions` : "Update extensions; distribution remains unchanged"}
  ${APP_NAME} update --models       Refresh model catalogs only
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update ${APP_NAME === "pi" ? "pi" : "self"}             ${SELF_UPDATE_ENABLED ? `Update ${APP_NAME} only` : "Unavailable in this build"}
`);
			return;
...
async function refreshModelCatalogs(): Promise<void> {
...
async function getSelfUpdatePlan(force: boolean, versionCheckUrl?: string): Promise<SelfUpdatePlan> {
	let latestRelease: Awaited<ReturnType<typeof getLatestPiRelease>>;
	try {
		latestRelease = await getLatestPiRelease(VERSION, { retry: true, url: versionCheckUrl });
	} catch (error: unknown) {
			...
			case "update": {
				...
				if (updateTargetIncludesSelf(target)) {
					...
					if (!selfUpdateEnabled) {
						console.error(chalk.yellow(`${APP_NAME} distribution updates are currently disabled.`));
						console.error(
							chalk.dim(`Use the installation or source-checkout process that provided this ${APP_NAME} build.`),
						);
						process.exitCode = 1;
						return true;
					}
					const selfUpdatePlan = await getSelfUpdatePlan(options.force, runtimeOptions.versionCheckUrl);
