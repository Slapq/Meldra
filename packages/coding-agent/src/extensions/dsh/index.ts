...
export default function dshExtension(pi: ExtensionAPI): void {
	...
	const waitForIdle = async (timeoutMs = 30000): Promise<boolean> => {
	...
	const updateModelBridgeStatus = (ctx: ExtensionContext, catalog: unknown): void => {
		...
		if (preference) {
			const routeMatches = provider === preference.provider && model === preference.id;
			label += routeMatches
				? " · MetaPi preference matched"
				: preferenceInCatalog
					? ` · MetaPi preference ${preference.provider}/${preference.id} · native priority`
					: ` · MetaPi preference ${preference.provider}/${preference.id} · unmatched`;
		}
		...
		handler: async (args, ctx) => {
			...
			if (action === "rewind") {
				...
				const choice = await ctx.ui.custom<DshRewindChoice | undefined>((_tui, _theme, _keybindings, done) => {
					...
					return new UserMessageSelectorComponent(
						...
						undefined,
						{
							...
							description:
								"Select a user turn to fork from its preceding completed boundary and restore its draft",
						},
					);
				});
				if (!choice) return;
				...
				for (const reference of choice.attachments) {
					...
					const loaded = await runtime.attachment(reference.attachmentId);
				...
				if (
					!(await ctx.ui.confirm(
						"回退并重写",
						`将从消息 #${choice.seq} 之前的已完成回合派生新 Session，源 Session ${sourceSessionId ?? "未知"} 保持不变${running ? "，当前运行将先取消" : ""}。是否继续？`,
					))
				)
					return;
				if (running) {
					...
					if (!(await waitForIdle()))
						return ctx.ui.notify("当前 Harness 回合仍在结束，暂时无法安全回退。请稍后重试。", "error");
				}
				const sessionId = await runtime.fork(choice.boundary);
				clearQueueSurface(ctx);
	...
	pi.on("session_start", (_event, ctx) => {
		...
		profileModelPreference = ctx.model ? { provider: ctx.model.provider, id: ctx.model.id } : undefined;
		...
		void runtime
			...
			.then(async (catalog) => {
				...
				const profileModel = profileModelPreference;
