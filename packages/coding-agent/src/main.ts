...
import { createProfileRuntime } from "./core/profile-agent-runtime.ts";
...
function commandNeedsModelsBeforeRuntime(args: string[]): boolean {
	if (args.includes("--help") || args.includes("-h")) return false;
	if (args[0] === "auth") return true;
...
async function handleMeldraSetupCommand(args: string[]): Promise<boolean> {
	...
	if (unknown) {
		console.error(chalk.red(`Error: Unknown setup option: ${unknown}`));
		process.exitCode = 1;
		return true;
	}
	if (options.has("--migrate") && options.has("--start-fresh")) {
		console.error(chalk.red("Error: Choose only one of --migrate or --start-fresh."));
		process.exitCode = 1;
		return true;
	}
...
async function runAuthCommand(
	args: string[],
	paths: { authPath: string; modelsPath: string; modelsStorePath: string },
): Promise<boolean> {
	...
	try {
		...
		try {
			const credentials = command.noRefresh
				? new ReadOnlyAuthStorage(paths.authPath)
				: AuthStorage.create(paths.authPath);
			const modelRuntime = await createAuthCheckModelRuntime(credentials, paths.modelsPath, paths.modelsStorePath);
...
async function findLocalSessionByExactId(
	...
	cwd: string,
	sessionDir?: string,
	listCurrent?: () => Promise<SessionInfo[]>,
): Promise<{ type: "local"; path: string } | undefined> {
	const localSessions = listCurrent ? await listCurrent() : await SessionManager.list(cwd, sessionDir);
...
async function resolveSessionPath(
	...
	const localSessions = loaders ? await loaders.listCurrent() : await SessionManager.list(cwd, sessionDir);
	const localMatch =
	...
	const allSessions = loaders ? await loaders.listAll() : await SessionManager.listAll(sessionDir);
	const globalMatch =
...
async function createSessionManager(
	...
	settingsManager: SettingsManager,
	loaders: {
		listCurrent: (onProgress?: (loaded: number, total: number) => void) => Promise<SessionInfo[]>;
	...
	if (parsed.fork) {
		if (parsed.sessionId) {
			const existingTarget = await findLocalSessionByExactId(parsed.sessionId, cwd, sessionDir, loaders.listCurrent);
			if (existingTarget) {
		...
		const resolved = await resolveSessionPath(parsed.fork, cwd, sessionDir, loaders);
	...
	if (parsed.session) {
		const resolved = await resolveSessionPath(parsed.session, cwd, sessionDir, loaders);
	...
	if (parsed.resume) {
		try {
			const selectedPath = await selectSession(loaders.listCurrent, loaders.listAll, settingsManager);
			if (!selectedPath) {
	...
	if (parsed.continue) {
		const sessions = await loaders.listCurrent();
		return sessions.length > 0 ? SessionManager.open(sessions[0].path) : SessionManager.create(cwd, sessionDir);
	}

	if (parsed.sessionId) {
		const existingSession = await findLocalSessionByExactId(parsed.sessionId, cwd, sessionDir, loaders.listCurrent);
		if (existingSession) {
...
export async function main(args: string[], options?: MainOptions) {
	...
	try {
		requestedProfile = extractProfileArgument(args);
	} catch (error) {
	...
	if (await handleProfileCommand(effectiveArgs, requestedProfile)) {
		return;
	}
	...
	const hostEnvironment = captureProfileEnvironment();
	const startupProfile = resolveProfile(launchCwd, requestedProfile);
	let activeProfileName = startupProfile.name;
	const getProfileState = (
		profileName: string,
		settingsCwd: string,
		options: { materializeModels?: boolean } = {},
	) => {
		const profile = resolveProfile(settingsCwd, profileName);
		...
		return { profile, record, baseSettings, modelPaths, missingEnvironment, runtime };
	};
	let startupProfileState = getProfileState(startupProfile.name, launchCwd, {
		materializeModels: commandNeedsModelsBeforeRuntime(effectiveArgs),
	});
	const selectedProfile = startupProfileState.profile;
	let profileBaseSettings = startupProfileState.baseSettings;
	const createProfileSettingsManager = (
		settingsCwd: string,
		options: Parameters<typeof SettingsManager.create>[2] = {},
	): SettingsManager => {
		const profile = resolveProfile(settingsCwd, activeProfileName);
		const record = readProfileRecord(profile.name);
		const importedSettings = record?.portable.settings ?? {};
		const baseSettings = profile.compatibility
			? importedSettings
	...
	const preliminaryModelPaths = startupProfileState.modelPaths;
	const missingProfileEnvironment = startupProfileState.missingEnvironment;
	if (missingProfileEnvironment.length > 0) {
		console.error(
			chalk.yellow(
				`Profile ${selectedProfile.name} is missing required environment variables: ${missingProfileEnvironment.join(", ")}`,
			),
		);
	}

	if (await runAuthCommand(effectiveArgs, preliminaryModelPaths)) {
		return;
	}
	...
	const cwd = launchCwd;
	let agentDir = startupProfileState.profile.agentDir;
	const bootstrapSettingsManager = createProfileSettingsManager(cwd, {
		projectTrusted: false,
	...
	if (await handleConfigCommand(effectiveArgs, { extensionFactories })) {
		return;
	}
	...
	if (
		...
		if (hasMigratablePiState()) {
			const selected = await showStartupSelector(
				createProfileSettingsManager(cwd, {
					projectTrusted: false,
					baseSettings: profileBaseSettings,
				}),
				"检测到现有 Pi 配置。是否一次性迁移到 Meldra？迁移后两者保持独立。",
	...
	startupProfileState = getProfileState(activeProfileName, cwd, {
		materializeModels: !parsed.help,
	});
	profileBaseSettings = startupProfileState.baseSettings;
	agentDir = startupProfileState.profile.agentDir;
	...
	const mergeSessions = (...collections: SessionInfo[][]): SessionInfo[] => {
	...
	let sessionManager = await createSessionManager(parsed, cwd, sessionDir, startupSettingsManager, {
		listCurrent: async (onProgress) =>
			mergeSessions(
				await SessionManager.list(cwd, sessionDir, onProgress),
				await listSessionsForCwdAcrossProfiles(cwd, activeProfileName),
			),
		listAll: async (onProgress) =>
			mergeSessions(
				await SessionManager.listAll(sessionDir, onProgress),
				await listSessionsAcrossProfiles(activeProfileName),
			),
	});
	const storedProfile = getSessionProfile(sessionManager);
	if (storedProfile) activeProfileName = storedProfile;
	...
	if (workspaceRoot && isFreshSession && sessionManager.buildSessionContext().messages.length === 0) {
		...
		sessionManager = sessionManager.isPersisted()
			? SessionManager.create(workspace.cwd, profileSessionDir, {
	...
	} else if (workspaceRoot && parsed.fork) {
		const sourceSession = sessionManager.getHeader()?.parentSession;
		if (!sourceSession) {
			throw new Error("Forked session is missing its source session path");
		}
		const sourceManager = SessionManager.open(sourceSession);
		const workspace = copyWorkspace(sourceManager.getCwd(), workspaceRoot, sessionManager.getSessionId());
		const profileSessionDir = getProfileSessionDir(workspace.cwd, activeProfileName);
		sessionManager = SessionManager.forkFrom(sourceSession, workspace.cwd, profileSessionDir, {
			id: sessionManager.getSessionId(),
		});
		setSessionProfile(sessionManager, activeProfileName);
		setSessionWorkspaceRoot(sessionManager, workspaceRoot);
	...
	const createRuntime: CreateAgentSessionRuntimeFactory = async ({
		cwd,
		agentDir: _previousAgentDir,
		sessionManager,
		sessionStartEvent,
		projectTrustContext,
		profileName,
	}) => {
		activeProfileName = profileName ?? getSessionProfile(sessionManager) ?? activeProfileName;
		if (!getSessionProfile(sessionManager)) {
			setSessionProfile(sessionManager, activeProfileName);
		}
		const runtimeProfileState = getProfileState(activeProfileName, cwd, {
			materializeModels: true,
		});
		agentDir = runtimeProfileState.profile.agentDir;
		profileBaseSettings = runtimeProfileState.baseSettings;
		const runtimeModelPaths = runtimeProfileState.modelPaths;
		if (runtimeProfileState.missingEnvironment.length > 0) {
			console.error(
				chalk.yellow(
					`Profile ${activeProfileName} is missing required environment variables: ${runtimeProfileState.missingEnvironment.join(", ")}`,
				),
			);
		}
		...
		const runtimeSettingsManager = createProfileSettingsManager(cwd, {
			projectTrusted,
			baseSettings: profileBaseSettings,
		});
		const services = await createAgentSessionServices({
			cwd,
			agentDir,
			authPath: runtimeModelPaths.authPath,
			modelsPath: runtimeModelPaths.modelsPath,
			modelsStorePath: runtimeModelPaths.modelsStorePath,
		...
		const profileRuntime = await createProfileRuntime(builtInProfileRuntimeProviders, {
			name: runtimeProfileState.profile.name,
			displayName: runtimeProfileState.profile.displayName,
			agentDir,
			cwd,
			compatibility: runtimeProfileState.profile.compatibility,
			runtime: runtimeProfileState.runtime,
			modelRuntime,
		});
	...
	const runtime = await createAgentSessionRuntime(createRuntime, {
		...
		meldraLifecycle: {
			...
			setWorkspaceRoot: setSessionWorkspaceRoot,
			getSessionDir: getProfileSessionDir,
			createEmptyWorkspace: (root, sessionId) => createEmptyWorkspace(root, sessionId).cwd,
			copyWorkspace: (sourceCwd, root, sessionId) => copyWorkspace(sourceCwd, root, sessionId).cwd,
		},
	});
