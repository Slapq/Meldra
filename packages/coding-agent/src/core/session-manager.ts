...
export class SessionManager {
	...
	createBranchedSession(
		leafId: string,
		options?: { cwd?: string; sessionDir?: string; id?: string },
	): string | undefined {
		...
		const targetCwd = resolvePath(options?.cwd ?? this.cwd);
		const targetSessionDir = normalizePath(options?.sessionDir ?? this.getSessionDir());
		if (this.persist && !existsSync(targetSessionDir)) mkdirSync(targetSessionDir, { recursive: true });
		const timestamp = new Date().toISOString();
		const fileTimestamp = timestamp.replace(/[:.]/g, "-");
		const newSessionFile = join(targetSessionDir, `${fileTimestamp}_${newSessionId}.jsonl`);
	...
	/** List all sessions stored under one agent directory. */
	static async listAllInAgentDir(agentDir: string, onProgress?: SessionListProgress): Promise<SessionInfo[]> {
		return SessionManager._listAllFromRoot(join(resolvePath(agentDir), "sessions"), onProgress);
	}
	...
	static async listAll(
		...
		return SessionManager._listAllFromRoot(getSessionsDir(), progress);
	}

	private static async _listAllFromRoot(sessionsDir: string, progress?: SessionListProgress): Promise<SessionInfo[]> {
		try {
