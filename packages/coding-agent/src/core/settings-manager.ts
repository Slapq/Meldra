...
import type { MeldraHooksSettings } from "../hooks/types.ts";
...
export interface Settings {
	...
	hooks?: MeldraHooksSettings; // Meldra command hooks keyed by lifecycle event
	disableAllHooks?: boolean; // Disable Profile and project Meldra hooks
...
export class FileSettingsStorage implements SettingsStorage {
	...
	constructor(cwd: string, agentDir: string, globalSettingsPath?: string) {
		const resolvedCwd = resolvePath(cwd);
		const resolvedAgentDir = resolvePath(agentDir);
		this.globalSettingsPath = globalSettingsPath
			? resolvePath(globalSettingsPath)
...
export class SettingsManager {
	...
	private mergeSettingsLayers(): Settings {
		return deepMergeSettings(deepMergeSettings(this.baseSettings, this.globalSettings), this.projectSettings);
	}
	...
	static fromStorage(storage: SettingsStorage, options: SettingsManagerCreateOptions = {}): SettingsManager {
		...
		return new SettingsManager(
			storage,
			structuredClone(options.baseSettings ?? {}),
			globalLoad.settings,
	...
	getEffectiveGlobalSettings(): Settings {
		return deepMergeSettings(this.baseSettings, this.globalSettings);
	}
