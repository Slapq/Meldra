    (function() {
      ...
      const { header, entries, leafId: defaultLeafId, systemPrompt, tools, renderedTools, renderedCustomEntries } = data;
      ...
      function renderEntry(entry) {
        ...
        if (entry.type === 'custom') {
          const rendered = renderedCustomEntries?.[entry.id];
          if (!rendered) return '';
          return `<div class="custom-entry" id="${entryDomId}">${copyBtnHtml}${tsHtml}<div class="custom-entry-content">${rendered}</div></div>`;
        }
      ...
      function computeStats(entryList) {
        ...
        for (const entry of entryList) {
          if (entry.type === 'message') {
            ...
            customMessages++;
          } else if (entry.type === 'custom' && renderedCustomEntries?.[entry.id]) {
            customMessages++;
          }
        }
