...
import type { CustomEntryHtmlRenderer } from "./custom-entry-renderer.ts";
...
export interface ExportOptions {
	...
	/** Optional renderer for extension-owned custom entries in an active Session. */
	customEntryRenderer?: CustomEntryHtmlRenderer;
}
...
interface SanitizedCustomEntry {
	type: "custom";
	id: string;
...
type HtmlSessionEntry = Exclude<SessionEntry, CustomEntry> | SanitizedCustomEntry;

interface SessionData {
	header: ReturnType<SessionManager["getHeader"]>;
	entries: HtmlSessionEntry[];
	leafId: string | null;
	systemPrompt?: string;
	tools?: Array<Pick<ToolDefinition, "name" | "description" | "parameters">>;
	/** Pre-rendered HTML for custom tool calls/results, keyed by tool call ID */
	renderedTools?: Record<string, RenderedToolHtml>;
	/** Pre-rendered extension custom entries, keyed by Session entry ID. */
...
function preRenderCustomEntries(entries: SessionEntry[], renderer: CustomEntryHtmlRenderer): Record<string, string> {
	const renderedEntries: Record<string, string> = {};
	for (const entry of entries) {
		if (entry.type !== "custom") continue;
		const html = renderer.render(entry);
		if (html) renderedEntries[entry.id] = html;
	}
	return renderedEntries;
}

/**
 * Custom entry payloads are extension-private state. HTML receives only the
 * structural fields needed to preserve branch navigation; visible content is
 * supplied separately by a registered renderer.
 */
function sanitizeEntriesForHtml(entries: SessionEntry[]): HtmlSessionEntry[] {
...
export async function exportSessionToHtml(
	...
	let renderedCustomEntries: Record<string, string> | undefined;
	if (opts.customEntryRenderer) {
		renderedCustomEntries = preRenderCustomEntries(entries, opts.customEntryRenderer);
		if (Object.keys(renderedCustomEntries).length === 0) {
			renderedCustomEntries = undefined;
		}
	}

	// Ordinary Pi sessions remain deferred until their first assistant response.
	// An active Profile Runtime may instead own a complete, renderable transcript
	// in custom entries without ever producing a Pi assistant message.
	if (!existsSync(sessionFile) && !renderedCustomEntries) {
	...
	const sessionData: SessionData = {
		header: sm.getHeader(),
		entries: sanitizeEntriesForHtml(entries),
		leafId: sm.getLeafId(),
		systemPrompt: state?.systemPrompt,
		tools: state?.tools?.map((t) => ({ name: t.name, description: t.description, parameters: t.parameters })),
		renderedTools,
		renderedCustomEntries,
	};
...
export async function exportFromFile(inputPath: string, options?: ExportOptions | string): Promise<string> {
	...
	const sessionData: SessionData = {
		header: sm.getHeader(),
		entries: sanitizeEntriesForHtml(sm.getEntries()),
		leafId: sm.getLeafId(),
