...
import { type CustomEntryHtmlRenderer, createCustomEntryHtmlRenderer } from "./export-html/custom-entry-renderer.ts";
...
import type { ProfileAgentRuntime } from "./profile-agent-runtime.ts";
...
export interface AgentSessionConfig {
	...
	/** Optional Profile-owned agent backend. Omitted for native Pi runtimes. */
...
export class AgentSession {
	...
	private readonly _profileRuntime?: ProfileAgentRuntime;
	...
	constructor(config: AgentSessionConfig) {
		...
		this._profileRuntime = config.profileRuntime;
	...
	async disposeProfileRuntime(): Promise<void> {
		await this._profileRuntime?.dispose();
	}
	...
	dispose(): void {
		...
		void this._profileRuntime?.dispose();
	...
	get isStreaming(): boolean {
		return this._profileRuntime?.isStreaming ?? this._isAgentRunActive;
	}
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		try {
			...
			if (this._profileRuntime) {
				await this._profileRuntime.prompt({
					text: expandedText,
	...
	async abort(): Promise<void> {
		if (this._profileRuntime) {
			await this._profileRuntime.abort();
			return;
		}
		this.abortRetry();
		this.agent.abort();
	...
	async waitForIdle(): Promise<void> {
		if (this._profileRuntime) {
			await this._profileRuntime.waitForIdle();
			return;
		}
		if (this.isIdle) {
			return;
	...
	async setModel(model: Model<any>): Promise<void> {
		...
		await this._profileRuntime?.selectModel?.(model);
	...
	private async _cycleScopedModel(direction: "forward" | "backward"): Promise<ModelCycleResult | undefined> {
		...
		await this._profileRuntime?.selectModel?.(next.model);

		// Apply model only after the Profile Runtime accepts the route.
	...
	private async _cycleAvailableModel(direction: "forward" | "backward"): Promise<ModelCycleResult | undefined> {
		...
		const thinkingLevel = this._getThinkingLevelForModelSwitch();
		await this._profileRuntime?.selectModel?.(nextModel);
	...
	private _bindExtensionCore(runner: ExtensionRunner): void {
		...
		runner.bindCore(
			...
			{
				getProfileRuntime: () => this._profileRuntime,
	...
	async exportToHtml(outputPath?: string, options: { themeName?: string } = {}): Promise<string> {
		...
		const customEntryRenderer: CustomEntryHtmlRenderer = createCustomEntryHtmlRenderer({
			getEntryRenderer: (customType) => this._extensionRunner.getEntryRenderer(customType),
			theme,
		});
	...
	getLastAssistantText(): string | undefined {
		if (this._profileRuntime?.getLastAssistantText) {
			return this._profileRuntime.getLastAssistantText();
		}
