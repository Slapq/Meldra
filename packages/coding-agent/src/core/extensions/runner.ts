...
export type SwitchProfileHandler = (profileName: string) => Promise<{ cancelled: boolean }>;
...
export class ExtensionRunner {
	...
	private getProfileRuntime: () => ProfileAgentRuntime | undefined = () => undefined;
	private getModel: () => Model<any> | undefined = () => undefined;
	private getScopedModels: () => readonly ScopedModel[] = () => [];
	private setModelPreferenceFn: ((provider: string, modelId: string) => void) | undefined;
	private isIdleFn: () => boolean = () => true;
	private isProjectTrustedFn: () => boolean = () => true;
	private getSignalFn: () => AbortSignal | undefined = () => undefined;
	private executeCommandHandler: (command: string) => Promise<boolean> = async () => false;
	...
	private switchProfileHandler: SwitchProfileHandler | undefined;
	...
	bindCore(
		...
		this.getProfileRuntime = contextActions.getProfileRuntime;
		this.setModelPreferenceFn = contextActions.setModelPreference;
	...
	bindCommandContext(actions?: ExtensionCommandContextActions): void {
		if (actions) {
			this.executeCommandHandler = actions.executeCommand;
			this.waitForIdleFn = actions.waitForIdle;
			this.newSessionHandler = actions.newSession;
			this.forkHandler = actions.fork;
			this.navigateTreeHandler = actions.navigateTree;
			this.switchSessionHandler = actions.switchSession;
			this.switchProfileHandler = actions.switchProfile;
			this.reloadHandler = actions.reload;
			return;
		}

		this.executeCommandHandler = async () => false;
		this.waitForIdleFn = async () => {};
		this.newSessionHandler = async () => ({ cancelled: false });
		this.forkHandler = async () => ({ cancelled: false });
		this.navigateTreeHandler = async () => ({ cancelled: false });
		this.switchSessionHandler = async () => ({ cancelled: false });
		this.switchProfileHandler = undefined;
	...
	createContext(): ExtensionContext {
		...
		return {
			...
			get profileRuntime() {
				runner.assertActive();
				return runner.getProfileRuntime();
			},
			get model() {
				runner.assertActive();
				return getModel();
			...
			setModelPreference: (provider, modelId) => {
				runner.assertActive();
				runner.setModelPreferenceFn?.(provider, modelId);
			},
			get scopedModels() {
				runner.assertActive();
	...
	createCommandContext(): ExtensionCommandContext {
		...
		context.executeCommand = (command) => {
			this.assertActive();
			return this.executeCommandHandler(command);
		};
		context.waitForIdle = () => {
			this.assertActive();
		...
		if (this.switchProfileHandler) {
			context.switchProfile = (profileName) => {
				this.assertActive();
				return this.switchProfileHandler!(profileName);
			};
		}
		context.reload = () => {
			this.assertActive();
