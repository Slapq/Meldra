...
export interface SessionBrowserItem extends SessionSearchRecord {
	/** Opaque provider-owned identity used for selection and hierarchy. */
	key: string;
	parentKey?: string;
	/** Real local path when the provider owns a file-backed Session. */
	path?: string;
	created: Date;
	messageCount?: number;
	firstMessage: string;
...
export type SessionBrowserLoader = (onProgress?: SessionListProgress) => Promise<SessionBrowserItem[]>;
...
class SessionSelectorHeader implements Component {
	...
	private confirmingDeleteKey: string | null = null;
	...
	setCapabilities(capabilities: { delete: boolean; path: boolean }): void {
		this.canDelete = capabilities.delete;
		this.canShowPath = capabilities.path;
	}
	...
	render(width: number): string[] {
		const scopeTitle = this.scope === "current" ? `${this.title} (Current Folder)` : `${this.title} (All)`;
		const leftText = theme.bold(scopeTitle);
		...
		if (this.confirmingDeleteKey !== null) {
			...
			if (this.canDelete) hint2Parts.push(keyHint("app.session.delete", "delete"));
			if (this.canShowPath) hint2Parts.push(keyHint("app.session.togglePath", `path ${pathState}`));
 ...
 */
function buildSessionTree(sessions: SessionBrowserItem[]): SessionTreeNode[] {
	const byKey = new Map<string, SessionTreeNode>();

	for (const session of sessions) {
		byKey.set(session.key, { session, children: [], latestActivity: session.modified.getTime() });
	}
...
class SessionList implements Component, Focusable {
	public getSelectedSessionKey(): string | undefined {
		const selected = this.filteredSessions[this.selectedIndex];
		return selected?.session.key;
	}
	private allSessions: SessionBrowserItem[] = [];
	...
	public onSelect?: (sessionKey: string) => void;
	...
	public onDeleteConfirmationChange?: (key: string | null) => void;
	public onDeleteSession?: (sessionKey: string) => Promise<void>;
	public onRenameSession?: (sessionKey: string) => void;
	...
	constructor(
		...
		this.searchInput.onSubmit = () => {
			if (this.filteredSessions[this.selectedIndex]) {
				const selected = this.filteredSessions[this.selectedIndex];
				if (this.onSelect) {
					this.onSelect(selected.session.key);
				}
			}
		};
	}
	...
	setSessions(sessions: SessionBrowserItem[], showCwd: boolean): void {
		this.allSessions = sessions;
		this.showCwd = showCwd;
		this.filterSessions(this.searchInput.getValue());
	}
	...
	private setConfirmingDeleteKey(key: string | null): void {
		this.confirmingDeleteKey = key;
		this.onDeleteConfirmationChange?.(key);
	}

	private startDeleteConfirmationForSelectedSession(): void {
		const selected = this.filteredSessions[this.selectedIndex];
		if (!selected || !this.onDeleteSession) return;

		if (this.isCurrentSessionKey(selected.session.key)) {
			this.onError?.("Cannot delete the currently active session");
			return;
		}

		this.setConfirmingDeleteKey(selected.session.key);
	}

	private isCurrentSessionKey(key: string): boolean {
		return this.currentSessionKey !== undefined && key === this.currentSessionKey;
	}
	...
	render(width: number): string[] {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			const isConfirmingDelete = session.key === this.confirmingDeleteKey;
			const isCurrent = this.isCurrentSessionKey(session.key);
			...
			const rightFacts = [...(session.badges ?? [])];
			if (session.messageCount !== undefined) rightFacts.push(String(session.messageCount));
			rightFacts.push(age);
			let rightPart = rightFacts.join(" ");
			if (this.showCwd && session.cwd) {
				rightPart = `${shortenPath(session.cwd)} ${rightPart}`;
			}
			if (this.showPath && session.path) {
	...
	handleInput(keyData: string): void {
		...
		if (this.confirmingDeleteKey !== null) {
			if (kb.matches(keyData, "tui.select.confirm")) {
				const keyToDelete = this.confirmingDeleteKey;
				this.setConfirmingDeleteKey(null);
				void this.onDeleteSession?.(keyToDelete);
				return;
			}
		...
		if (kb.matches(keyData, "app.session.rename")) {
			const selected = this.filteredSessions[this.selectedIndex];
			if (selected) {
				this.onRenameSession?.(selected.session.key);
			}
			return;
		}
...
type SessionsLoader = SessionBrowserLoader;
 ...
 */
export class SessionBrowserComponent extends Container implements Focusable {
	...
	private sortMode: SortMode = "threaded";
	private nameFilter: NameFilter = "all";
	private currentSessions: SessionBrowserItem[] | null = null;
	private allSessions: SessionBrowserItem[] | null = null;
	private currentSessionsLoader: SessionsLoader;
	private allSessionsLoader: SessionsLoader;
	private requestRender: () => void;
	private renameSession?: (sessionKey: string, currentName: string | undefined) => Promise<void>;
	...
	constructor(
		...
		options?: {
			title?: string;
			renameSession?: (sessionKey: string, currentName: string | undefined) => Promise<void>;
			deleteSession?: (sessionKey: string) => Promise<{ ok: boolean; method?: "trash" | "unlink"; error?: string }>;
		...
		this.header.setCapabilities({ delete: !!this.deleteSession, path: options?.showPathHint === true });
		...
		this.sessionList.onSelect = (sessionKey) => {
			clearStatusMessage();
			onSelect(sessionKey);
		};
		...
		this.sessionList.onRenameSession = (sessionKey) => {
			...
			this.enterRenameMode(sessionKey, session?.name);
		};
		...
		this.sessionList.onDeleteConfirmationChange = (key) => {
			this.header.setConfirmingDeleteKey(key);
			this.requestRender();
		};
		...
		if (this.deleteSession) {
			this.sessionList.onDeleteSession = async (sessionKey: string) => {
				const result = await this.deleteSession!(sessionKey);

				if (result.ok) {
					if (this.currentSessions) {
						this.currentSessions = this.currentSessions.filter((session) => session.key !== sessionKey);
					}
					if (this.allSessions) {
						this.allSessions = this.allSessions.filter((session) => session.key !== sessionKey);
					}
	...
	private enterRenameMode(sessionKey: string, currentName: string | undefined): void {
		this.mode = "rename";
		this.renameTargetKey = sessionKey;
	...
	private async confirmRename(value: string): Promise<void> {
		...
		if (!next) return;
		const target = this.renameTargetKey;
		if (!target) {
			this.exitRenameMode();
			return;
		}
...
function toSessionBrowserItem(session: SessionInfo): SessionBrowserItem {
	return {
		key: canonicalizePath(session.path) ?? session.path,
		...(session.parentSessionPath
			? { parentKey: canonicalizePath(session.parentSessionPath) ?? session.parentSessionPath }
			: {}),
		path: session.path,
		id: session.id,
		cwd: session.cwd,
...
/** Pi file-backed Session adapter over the product-neutral browser. */
export class SessionSelectorComponent extends SessionBrowserComponent {
	constructor(
		...
		const wrapLoader =
			(loader: (onProgress?: SessionListProgress) => Promise<SessionInfo[]>): SessionBrowserLoader =>
			async (onProgress) =>
				(await loader(onProgress)).map((session) => {
					const item = toSessionBrowserItem(session);
					paths.set(item.key, session.path);
					return item;
				});
		const pathFor = (key: string): string => paths.get(key) ?? key;
		super(
			wrapLoader(currentSessionsLoader),
			wrapLoader(allSessionsLoader),
			(key) => onSelect(pathFor(key)),
			onCancel,
			onExit,
			requestRender,
			{
				...
				...(options?.renameSession
					? {
							renameSession: (key: string, name: string | undefined) =>
								options.renameSession!(pathFor(key), name),
						}
					: {}),
				deleteSession: (key) => deleteSessionFile(pathFor(key)),
				showRenameHint: options?.showRenameHint,
				showPathHint: true,
				keybindings: options?.keybindings,
			},
			canonicalizePath(currentSessionFilePath),
		);
	}
}
