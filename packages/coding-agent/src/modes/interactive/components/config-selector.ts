...
class ConfigSelectorHeader implements Component {
	...
	render(width: number): string[] {
		const title = theme.bold(
			this.writeScope === "project" ? "Current WorkSpace Resources" : "Current Profile Resources",
		);
		...
		const scopeHint =
			this.writeScope === "project"
				? theme.fg("muted", `${CONFIG_DIR_NAME}/settings.json · inherited Profile resources are dimmed`)
				: theme.fg("muted", "Current Profile agent/settings.json");
