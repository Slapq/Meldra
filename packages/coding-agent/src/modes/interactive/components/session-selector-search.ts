...
function getSessionSearchText(session: SessionSearchRecord): string {
	return `${session.id} ${session.name ?? ""} ${session.allMessagesText} ${session.cwd}`;
}

export function hasSessionName(session: SessionSearchRecord): boolean {
	return Boolean(session.name?.trim());
}

function matchesNameFilter(session: SessionSearchRecord, filter: NameFilter): boolean {
...
export function matchSession(session: SessionSearchRecord, parsed: ParsedSearchQuery): MatchResult {
...
export function filterAndSortSessions<T extends SessionSearchRecord>(
	sessions: T[],
	query: string,
	sortMode: SortMode,
