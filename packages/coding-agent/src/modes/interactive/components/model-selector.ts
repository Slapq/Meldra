...
export class ModelSelectorComponent extends Container implements Focusable {
	...
	private onSelectCallback: (model: Model<any>) => Promise<void> | void;
	...
	constructor(
		...
		onSelect: (model: Model<any>) => Promise<void> | void,
		onCancel: () => void,
	...
	private updateList(): void {
		...
		if (this.selectionPending) {
			this.listContainer.addChild(new Spacer(1));
			this.listContainer.addChild(new Text(theme.fg("accent", "  Selecting model…"), 0, 0));
	...
	private async handleSelect(model: Model<any>): Promise<void> {
		if (this.closed || this.selectionPending) return;
		this.selectionPending = true;
		this.updateList();
		this.tui.requestRender();
		try {
			await this.onSelectCallback(model);
		} finally {
