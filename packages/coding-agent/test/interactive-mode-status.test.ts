...
describe("InteractiveMode.refreshModelPresentation", () => {
	test("forces the footer to render after an asynchronous model commit", () => {
		const fakeThis: any = {
		...
		(InteractiveMode as any).prototype.refreshModelPresentation.call(fakeThis);
...
describe("InteractiveMode.createExtensionUIContext input compatibility", () => {
	test("keeps input unchanged and adds a masked secretInput path", async () => {
		const fakeThis: any = {
			showExtensionInput: vi.fn(async () => "value"),
			ui: { terminal: { setTitle: vi.fn() } },
		};
		const uiContext = (InteractiveMode as any).prototype.createExtensionUIContext.call(fakeThis);
		const options = { timeout: 1000 };

		await uiContext.input("Plain", "placeholder", options);
		await uiContext.secretInput("Secret", "credential", options);

		expect(fakeThis.showExtensionInput).toHaveBeenNthCalledWith(1, "Plain", "placeholder", options);
		expect(fakeThis.showExtensionInput).toHaveBeenNthCalledWith(2, "Secret", "credential", options, "*");
	});
});
...
describe("InteractiveMode.createBaseAutocompleteProvider", () => {
	test("matches model command arguments across provider/model order", async () => {
		...
		fakeThis.isProfileHiddenBuiltinCommand = (name) => name === "clone";
		const hiddenProvider = createBaseAutocompleteProvider.call(fakeThis);
		const hiddenSuggestions = await hiddenProvider.getSuggestions(["/clone"], 0, 6, {
			signal: new AbortController().signal,
		});
		expect(hiddenSuggestions?.items.some((item) => item.value === "clone") ?? false).toBe(false);
	});

	test("shows a Profile-preferred extension command instead of its built-in autocomplete entry", async () => {
		type FakeInteractiveMode = {
			session: {
				scopedModels: [];
		...
		const fakeThis: FakeInteractiveMode = {
					...
					getRegisteredCommands: () => [
						{
							...
							description: "Select Harness native model",
							sourceInfo: undefined,
						},
					],
				},
			...
			sessionManager: { getCwd: () => "/tmp" },
			fdPath: null,
			isProfilePreferredExtensionCommand: (name) => name === "model",
			isProfileHiddenBuiltinCommand: () => false,
			prefixAutocompleteDescription: (description) => description,
		};

		const provider = createBaseAutocompleteProvider.call(fakeThis);
		const suggestions = await provider.getSuggestions(["/mod"], 0, 4, {
			signal: new AbortController().signal,
		});
