...
import type { ProfileAgentRuntime } from "../../src/core/profile-agent-runtime.ts";
import type { BuildSystemPromptOptions, ExtensionAPI } from "../../src/index.ts";
import { createHarness, getAssistantTexts, type Harness } from "./harness.ts";

function externalRuntime(selectModel: ProfileAgentRuntime["selectModel"]): ProfileAgentRuntime {
	return {
		isStreaming: false,
		attach: vi.fn(),
...
describe("AgentSession model and extension characterization", () => {
	...
	it("lets a Profile Runtime accept the model before committing the Pi selection", async () => {
		...
		const harness = await createHarness({
			...
			profileRuntime: externalRuntime(selectModel),
		});
		harnesses.push(harness);
		const nextModel = harness.getModel("faux-2")!;
		...
		expect(selectModel).toHaveBeenCalledWith(nextModel);
		expect(harness.session.model?.id).toBe("faux-2");
	...
	it("does not commit a Pi model when the Profile Runtime rejects it", async () => {
		const selectModel = vi.fn(async () => {
			throw new Error("external model unavailable");
		});
		const harness = await createHarness({
			models: [
				{ id: "faux-1", name: "One", reasoning: true },
		...
		await expect(harness.session.setModel(harness.getModel("faux-2")!)).rejects.toThrow("external model unavailable");

		expect(harness.session.model?.id).toBe("faux-1");
		expect(harness.sessionManager.getEntries().some((entry) => entry.type === "model_change")).toBe(false);
	});

	it("lets a Profile Runtime accept a cycled model before committing the Pi selection", async () => {
		const selectModel = vi.fn(async () => undefined);
		const harness = await createHarness({
			models: [
				{ id: "faux-1", name: "One", reasoning: true },
	...
	it("does not commit a cycled Pi model when the Profile Runtime rejects it", async () => {
		const selectModel = vi.fn(async () => {
			throw new Error("external model unavailable");
		});
		const harness = await createHarness({
		...
		await expect(harness.session.cycleModel()).rejects.toThrow("external model unavailable");
	...
	it("cycles through scoped models and preserves the scoped thinking preference", async () => {
		...
		expect(selectModel).toHaveBeenLastCalledWith(modelTwo);

		await harness.session.cycleModel();
		expect(harness.session.model?.id).toBe("faux-1");
		expect(harness.session.thinkingLevel).toBe("high");
		expect(selectModel).toHaveBeenLastCalledWith(modelOne);
	});
