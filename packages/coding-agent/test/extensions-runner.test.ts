...
describe("ExtensionRunner", () => {
	...
	describe("command context", () => {
		it("passes command execution through to the bound handler", async () => {
			const runtime = createExtensionRuntime();
			const runner = new ExtensionRunner([], runtime, tempDir, sessionManager, modelRegistry);
			const executeCommand = vi.fn(async (command: string) => command === "/model");
			...
			await expect(runner.createCommandContext().executeCommand("/model")).resolves.toBe(true);
			expect(executeCommand).toHaveBeenCalledWith("/model");
		});

		it("returns false for command execution without an interactive host", async () => {
			const runtime = createExtensionRuntime();
			const runner = new ExtensionRunner([], runtime, tempDir, sessionManager, modelRegistry);
			runner.bindCommandContext();
			await expect(runner.createCommandContext().executeCommand("/model")).resolves.toBe(false);
		});
