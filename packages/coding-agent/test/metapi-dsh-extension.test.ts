...
describe("DSH Profile TUI renderer", () => {
	...
	it("shows when the Harness route matches the MetaPi Profile preference", async () => {
		const fake = fakeRuntime();
		const state = setup("dsh", fake.runtime);
		...
		await vi.waitFor(() =>
			expect(state.ctx.ui.setStatus).toHaveBeenCalledWith(
				"metapi-dsh-0-model",
				"Harness native deepseek-official/deepseek-v4-flash · MetaPi preference matched",
			),
		);
	...
	it("distinguishes native route priority from an unmatched Profile preference", async () => {
		const fake = fakeRuntime();
		const state = setup("dsh", fake.runtime);
		(state.ctx as unknown as { model: { provider: string; id: string } }).model = {
			provider: "deepseek-official",
		...
		await vi.waitFor(() =>
			expect(state.ctx.ui.setStatus).toHaveBeenCalledWith(
				"metapi-dsh-0-model",
				"Harness native deepseek-official/deepseek-v4-flash · MetaPi preference deepseek-official/deepseek-v4 · native priority",
			),
		);
		expect(fake.runtime.selectModel).not.toHaveBeenCalled();
		...
		await vi.waitFor(() =>
			expect(state.ctx.ui.setStatus).toHaveBeenCalledWith(
				"metapi-dsh-0-model",
				"Harness native deepseek-official/deepseek-v4-flash · MetaPi preference missing-provider/missing-model · unmatched",
			),
		);
		vi.unstubAllEnvs();
	});
