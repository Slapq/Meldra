...
export interface InputOptions {
	/** Optional single-column character used only when rendering the stored value. */
	mask?: string;
}
...
export class Input implements Component, Focusable {
	...
	render(width: number): string[] {
		...
		let visibleText = "";
		const displayValue = this.mask ? [...segmenter.segment(this.value)].map(() => this.mask).join("") : this.value;
