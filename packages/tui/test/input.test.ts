...
describe("Input component", () => {
	...
	describe("render", () => {
		it("masks rendering without changing the stored or submitted value", () => {
			const input = new Input({ mask: "*" });
			let submitted: string | undefined;
			input.onSubmit = (value) => {
				submitted = value;
			};
			input.setValue("s密cret");
			input.focused = true;

			const [line] = input.render(20);
			assert.ok(line);
			assert.ok(!line.includes("s"));
			assert.ok(!line.includes("密"));
			assert.ok(!line.includes("cret"));
			assert.strictEqual(input.getValue(), "s密cret");

			input.handleInput("\r");
			assert.strictEqual(submitted, "s密cret");
		});

		it("preserves the original unmasked rendering path", () => {
			const original = new Input();
			const explicitDefault = new Input({});
			original.setValue("plain-input");
			explicitDefault.setValue("plain-input");
			assert.deepStrictEqual(original.render(20), explicitDefault.render(20));
		});
