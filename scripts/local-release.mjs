...
import { runtimeInstallDependencies, runtimeOverrides } from "./prepare-native-runtime.mjs";
...
Options:
  ...
  --offline-model-data Use the model catalog already committed in the repository
  --skip-binary        Do not build the standalone Bun binary
...
function createCliShim(installDirectory) {
	...
	for (const command of ["metapi", "pi"]) {
		...
		if (existsSync(join(binDirectory, command))) {
...
// Public releases refresh model data by default. Reproducible downstream
// installers can explicitly package the catalog already committed in the tree.
...
if (!options.skipInstall) {
	...
	const dependencies = {
		...Object.fromEntries(packages.map((pkg) => [pkg.name, fileSpecifier(nodeInstallDirectory, tarballs.get(pkg.name))])),
		...runtimeInstallDependencies(repoRoot),
	};
	const overrides = { ...runtimeOverrides(repoRoot), ...dependencies };
	const installPackageJson = `${JSON.stringify({ private: true, dependencies, overrides }, undefined, "\t")}\n`;
	...
	run(process.execPath, [join(repoRoot, "scripts/prepare-native-runtime.mjs"), "--cwd", nodeInstallDirectory], {
		shell: false,
	});
	cliCommand = createCliShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		bunCliCommand = createCliShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? `${cliCommand}.cmd` : cliCommand)} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? `${bunCliCommand}.cmd` : bunCliCommand)} --help`);
	}
}
