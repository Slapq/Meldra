...
| ⚙️ | **Unified plugin configuration** | Ordinary Profiles receive the `/config` TUI; Provider and Scout onboarding is part of the Setup plan |
...
|---|---|---|
| Run from source | `SUPPORTED` | The only reliable entry today |
| `MetaPi-Setup.exe` | `PLANNED` | Planned double-click Windows installer; no download exists yet |
| Scoped npm Bootstrap | `PLANNED` | The MetaPi organization and package name are unconfirmed; no fake command is provided |
| Starter Bundle + onboarding | `PLANNED` | Will guide Provider, model, Scout, and thinking level; users can skip it |
| Windows desktop shortcut | `PLANNED` | Planned to launch `metapi --profile default --workspace` after installation |
...
> - DeepSeek Harness is pinned to `0.1.0-rc.6`; its release-candidate protocol may change.
